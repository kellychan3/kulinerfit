...
def add_resep():
    try:
        ...
        new_resep_id = resep_ref.id

        # Update data resep untuk menambahkan field "id"
        resep_ref.update({"id": new_resep_id})
...
def get_rekomendasi(userId):
    ...
    if user_data.exists:
        ...
        if favorit is not None and favorit != "-":
            favorit = favorit.title()
        print(favorit)
        alergi_before = user_data.get("alergi")
        if alergi_before is not None:
