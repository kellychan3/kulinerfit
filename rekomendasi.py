...
def get_recommendations(makanan_favorit, alergi=None, penyakit=None, resep_df=None, larangan_makanan=None):
    ...
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
     ...
     # Find the index of the current title in the sorted list
    ...
    # Move the current title to the beginning of the list
    if current_title_idx is not None:
        sim_scores.pop(current_title_idx)
        sim_scores.insert(0, (idx, 1.0))  # Set similarity score to 1.0 for the current title

    # Get the scores of the 5 most similar food
    sim_scores = sim_scores[1:11]
    # Extract the indices from sim_scores and add the current title's index at the beginning
    food_indices = [index for index, _ in sim_scores]
    # Add the current title's index to the beginning of the list
    food_indices.insert(0, idx)
